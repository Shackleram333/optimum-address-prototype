...
// LIVE address search via Photon (OpenStreetMap). Keyless + CORS-enabled, so it
// can be called directly from this static prototype with no API key or backend.
// Public instance: https://photon.komoot.io — fine for prototype-level traffic.
...
// Turn a Photon GeoJSON feature into the prototype's { line1, line2, value }
// shape. Returns null for features without a usable street/place name.
function featureToSuggestion(feature) {
  ...
  const cityLine = [locality, p.state, p.postcode].filter(Boolean).join(", ");
  const line2 = [cityLine, p.country].filter(Boolean).join(", ");
  return {
    line1,
    line2: line2 || p.country || "",
    value: line2 ? `${line1}, ${line2}` : line1,
...
async function fetchPhoton(query) {
  ...
  const url =
    `${PHOTON_ENDPOINT}?q=${encodeURIComponent(query)}&limit=8&lang=en`;
  const res = await fetch(url, { signal: searchAbort.signal });
  if (!res.ok) throw new Error(`Photon ${res.status}`);
  const data = await res.json();
  const all = (data.features || []).map(featureToSuggestion).filter(Boolean);
...
// data would require Optimum's serviceability backend). Four units per floor.
const SIM_UNIT_COUNT = 24;
function buildUnitOptions(count) {
  const total = Number(count) > 0 ? Number(count) : SIM_UNIT_COUNT;
...
// Live results carry no unit count, so the dropdown rows stay clean (no
// "(N units...)" label). The unit picker is shown after selection instead.
...
function requestSuggestions(rawValue) {
  ...
  searchTimer = setTimeout(async () => {
    try {
      ...
      setDropdownMode("helper");
      if (dropdownTitle)
        dropdownTitle.textContent = "Couldn't reach address search — check your connection.";
      updatePredictions();
    }
  }, 250);
}
...
checkPlansBtn.addEventListener("click", () => {
  ...
  // Address chosen but no unit yet → CTA acts as "Select a unit".
  if (selectedSuggestion && !selectedUnit) {
    setFocusState(true);
