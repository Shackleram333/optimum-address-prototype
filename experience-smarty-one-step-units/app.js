...
async function fetchSmarty(query) {
  ...
  // ONE-STEP UNITS: expand every multi-unit building inline so the individual
  // apartments appear directly in the dropdown (instead of a single "(N units...)"
  // row that opens a separate picker). Capped at 10 units per building (a single
  // Smarty call). Expansions run in parallel; if one fails we keep the building row.
  const lists = await Promise.all(
    base.map(async (s) => {
      ...
      try {
        ...
        if (labels.length) {
          return labels.map((u) => {
            ...
            return {
              ...
              line2: s.line2,
              value: `${line1}, ${s.line2}`,
              units: 0, // a specific apartment — a complete address, not an MDU
              unit: u,
              raw: s.raw,
            };
          });
        ...
        /* expansion failed — fall through to the plain building row */
      }
      return [s];
    })
  );
