...
// LIVE US address search via Melissa Global Express Entry. Express Entry is
// designed to be called directly from the browser; the license key is supplied
// via config.js (window.MELISSA_LICENSE_KEY) and is secured by domain-locking it
// in the Melissa portal. Like Smarty, Melissa returns real secondary/unit data
// (apartments). With suitecompression=true a multi-unit building returns ALL of
// its units in one SubBuilding comma-list in a single call, which we split into
// individual apartment rows inline (no second request needed).
const MELISSA_KEY = ((window.MELISSA_LICENSE_KEY || "") + "").trim();
const MELISSA_ENDPOINT =
  "https://expressentry.melissadata.net/web/GlobalExpressFreeForm";

function melissaKeyConfigured() {
  return MELISSA_KEY.length > 0;
}
...
// Build the prototype's { line1, line2, value, units, unit, raw } shape from a
// Melissa address record plus a specific unit label (empty string = no unit).
function melissaToSuggestion(addr, unit) {
  const street = (addr.Address1 || "").trim();
  const line1 = unit ? `${street} ${unit}`.trim() : street;
  // Address2 is already "City, ST ZIP"; fall back to assembling it if absent.
  const line2 =
    (addr.Address2 || "").trim() ||
  ...
  return {
    ...
    line2,
    value: `${line1}, ${line2}`,
    units: 0, // a specific, complete address — one-step rows never re-expand
    unit: unit || "",
    raw: addr,
  };
}

async function fetchMelissa(query) {
  ...
  const params = new URLSearchParams({
    ...
    ff: stripCountry(query),
  });
  const res = await fetch(`${MELISSA_ENDPOINT}?${params.toString()}`, {
    signal: searchAbort.signal,
  });
  if (!res.ok) throw new Error(`Melissa ${res.status}`);
  const data = await res.json();

  // Surface an explicit service error (e.g. bad/over-limit key) as "no results"
  // rather than throwing, so the dropdown shows the helper text.
  if (data.ErrorString) return [];
  ...
  results.forEach((r) => {
    ...
    if (!addr) return;
    const sub = (addr.SubBuilding || "").trim();
    const units = sub
...
// In the one-step flow every apartment is already expanded into its own row, so
// `units` is always 0 and no row re-expands into a separate unit picker. Kept for
// the shared dropdown/CTA logic that the curated demos also use.
...
function selectSuggestion(suggestion) {
  ...
  // ONE-STEP: the apartment (if any) is already baked into the suggestion's
  // value/line1, so we confirm on the full address — there is no separate unit
  // step (UNIT_NONE means "complete address, no further selection needed").
  ...
  showKeyboard(false);
  // Tapping a result (address or specific apartment) goes straight to plans.
  enterCheckingStep();
}
...
// Strip a trailing country token so Melissa autocomplete still matches when the
...
function requestSuggestions(rawValue) {
  ...
  if (!melissaKeyConfigured()) {
    latestResults = [];
    renderDropdownRows([]);
    setDropdownMode("helper");
    if (dropdownTitle)
      dropdownTitle.textContent =
        "Add your Melissa license key in config.js to enable live search.";
    updatePredictions();
    return;
  }
  ...
  searchTimer = setTimeout(async () => {
    try {
      const results = await fetchMelissa(query);
